...
def _validate_archive(zip_file: ZipFile) -> None:
    ...
    if len(infos) > MAX_ARCHIVE_FILES:
        raise ManifestError(
            f'扩展包内文件数量过多（{len(infos)} 超过 {MAX_ARCHIVE_FILES}），已拒绝！'
        )
    total_size = 0
    for info in infos:
        ...
        if not info.is_dir():
            ...
            if total_size > MAX_ARCHIVE_TOTAL:
                raise ManifestError(
                    f'扩展包解压后体积过大（超过 {MAX_ARCHIVE_TOTAL // (1024 * 1024)} MB），已拒绝！'
                )
...
def _read_manifest_from_zip(zip_file: ZipFile) -> ExtensionManifest:
    ...
    manifest_names = [
        name for name in zip_file.namelist() if name.endswith('Extension.toml')
    ]
