...
async def get_renderers(current_user: dict = Depends(get_current_user)):
    """返回已注册渲染引擎列表（含当前选中）。"""
    renderers = [{'name': name, 'current': name == config.image.renderer} for name in extension_manager.renderers]
...
async def get_themes(current_user: dict = Depends(get_current_user)):
    """返回可用主题列表（含当前选中）。"""
    themes = [{'name': theme_id, 'current': theme_id == config.image.theme} for theme_id in extension_manager.themes]
