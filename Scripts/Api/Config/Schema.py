...
ENV_SCHEMA = [
    ...
    {'key': 'SUPERUSERS', 'label': '超级用户', 'type': 'list', 'default': [], 'description': '拥有管理权限的用户标识（如 QQ 号）'},
    ...
    {'key': 'DRIVER', 'label': 'NoneBot 驱动', 'type': 'string', 'default': '~fastapi', 'description': 'NoneBot 运行所需的驱动组合，安装/卸载适配器时会自动维护'},
    # ===== OneBot V11 =====
    {'key': 'ONEBOT_ACCESS_TOKEN', 'label': 'OneBot AccessToken', 'type': 'secret', 'default': '', 'description': 'OneBot V11 平台的 AccessToken，由 OneBot 服务端设置；未启用请留空'},
    # ===== QQ =====
    {'key': 'QQ_BOTS', 'label': 'QQ 机器人', 'type': 'json', 'default': [], 'description': 'QQ 官方机器人列表（JSON 数组），每项：id（AppID）、token、secret、intent（事件订阅对象）、use_websocket（默认 True，False 使用 WebHook）'},
    {'key': 'QQ_IS_SANDBOX', 'label': 'QQ 沙盒模式', 'type': 'boolean', 'default': False, 'description': '是否启用 QQ 沙盒模式，默认关闭'},
    # ===== Telegram =====
    {'key': 'TELEGRAM_BOTS', 'label': 'Telegram 机器人', 'type': 'json', 'default': [], 'description': 'Telegram 机器人列表（JSON 数组），每项：token（向 @BotFather 申请，必填）、is_webhook（是否启用 Webhook 模式，默认 False）'},
    {'key': 'TELEGRAM_WEBHOOK_URL', 'label': 'Telegram Webhook 地址', 'type': 'string', 'default': '', 'description': 'Webhook 模式下的公网 HTTPS 地址，例如 https://yourdomain.com；Long polling 模式留空'},
    {'key': 'TELEGRAM_PROXY', 'label': 'Telegram 代理', 'type': 'string', 'default': '', 'description': '访问 Telegram API 的代理地址，例如 http://127.0.0.1:10809；Socks 协议需安装 httpx[socks]'},
    # ===== Discord =====
    {'key': 'DISCORD_BOTS', 'label': 'Discord 机器人', 'type': 'json', 'default': [], 'description': 'Discord 机器人列表（JSON 数组），每项：token（必填）、intent（事件订阅对象，message_content 为特权 Intent）、application_commands（斜杠命令注册范围，如 {"*": ["*"]}）'},
    {'key': 'DISCORD_API_VERSION', 'label': 'Discord API 版本', 'type': 'number', 'default': 10, 'description': 'Discord API 版本号，默认 10'},
    {'key': 'DISCORD_API_TIMEOUT', 'label': 'Discord API 超时', 'type': 'number', 'default': 30, 'description': 'Discord API 请求超时时间（秒），默认 30'},
    {'key': 'DISCORD_COMPRESS', 'label': 'Discord 数据压缩', 'type': 'boolean', 'default': False, 'description': '是否启用网关数据压缩，默认关闭'},
    {'key': 'DISCORD_HANDLE_SELF_MESSAGE', 'label': 'Discord 处理自身消息', 'type': 'boolean', 'default': False, 'description': '是否处理自己发送的消息，默认关闭'},
    {'key': 'DISCORD_PROXY', 'label': 'Discord 代理', 'type': 'string', 'default': '', 'description': '访问 Discord API 的代理地址，例如 http://127.0.0.1:6666；不使用请留空'},
    # ===== DoDo =====
    {'key': 'DODO_BOTS', 'label': 'DoDo 机器人', 'type': 'json', 'default': [], 'description': 'DoDo 开放平台机器人列表（JSON 数组，仅支持 WebSocket 连接），每项：client_id（必填）、token（必填）'},
    # ===== KOOK =====
    {'key': 'KAIHEILA_BOTS', 'label': 'KOOK 机器人', 'type': 'json', 'default': [], 'description': 'KOOK（开黑啦）机器人列表（JSON 数组），每项：token（开发者平台获取，必填）'},
    # ===== Satori =====
    {'key': 'SATORI_CLIENTS', 'label': 'Satori 服务', 'type': 'json', 'default': [], 'description': 'Satori 客户端列表（JSON 数组），每项：host、port、path（默认空）、token（视服务端要求）、timeout（默认 30）、secure（是否 TLS，默认 False）'},
    # ===== Minecraft =====
    {'key': 'MINECRAFT_WS_URLS', 'label': 'Minecraft WS 地址', 'type': 'json', 'default': {}, 'description': 'Minecraft WebSocket 连接地址（JSON）：{"服务器名称": ["ws://地址:端口/路径"]}'},
    {'key': 'MINECRAFT_ACCESS_TOKEN', 'label': 'Minecraft 令牌', 'type': 'secret', 'default': '', 'description': 'Minecraft WebSocket 连接令牌，未设置请留空'},
]
...
ENV_GROUPS = [
    ...
    {
        'name': 'Discord',
        'keys': ['DISCORD_BOTS', 'DISCORD_API_VERSION', 'DISCORD_API_TIMEOUT', 'DISCORD_COMPRESS', 'DISCORD_HANDLE_SELF_MESSAGE', 'DISCORD_PROXY'],
    },
    {
        'name': 'DoDo',
        'keys': ['DODO_BOTS'],
