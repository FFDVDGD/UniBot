...
| 功能 | 说明 |
|------|------|
| **::fluent-color:data-pie-24:: 仪表盘** | 实时查看运行状态、内存占用、在线服务器、绑定玩家数；含「快速开始」引导卡片 |
| **::fluent-color:flag-24:: 快速开始引导** | 三步完成平台接入、服务器接入与超级用户授权，支持令牌一键授权 |
| **::fluent-color:settings-24:: 配置管理** | 可视化编辑 `Config.toml` 和 `.env`，支持 Schema 校验与分组展示 |
| **::fluent-color:toolbox-24:: 扩展管理** | 查看与启停扩展、调整扩展配置、切换渲染引擎与模板 |
| **::fluent-color:clipboard-24:: 服务器管理** | 查看所有连接服务器状态、在线玩家，远程执行指令 |
| **::fluent-color:people-24:: 玩家管理** | 管理白名单绑定关系 |
| **::fluent-color:laptop-24:: 服务器详情** | 单服在线玩家、状态详情，远程执行指令 |
| **::fluent-color:puzzle-piece-24:: 适配器 / 插件** | 查看已安装平台适配器与已加载插件列表 |
| **::fluent-color:person-24:: 用户管理** | 管理 WebUI 登录账户与角色权限 |
| **::fluent-color:document-24:: 日志查看** | 实时滚动查看日志，支持分级筛选 |
| **::fluent-color:lock-closed-24:: 登录认证** | JWT + 密码认证（HttpOnly Cookie） |
:::
...
登录 WebUI 后，仪表盘顶部会显示「快速开始」引导卡片，按部署进度展示三个步骤，点击即可跳转或操作：

::: table title="快速开始步骤" copy="all"
| 步骤 | 完成条件 | 操作 |
|------|----------|------|
| **连接聊天平台** | 已加载聊天平台适配器（排除 Minecraft 适配器） | 前往适配器页安装 / 启用 |
| **接入 Minecraft 服务器** | 有在线服务器，或已配置 WS 地址 | 查看鹊桥安装文档 |
| **添加超级用户** | `SUPERUSERS` 非空 | 点击「令牌授权」弹出令牌提示框 |
:::

全部步骤完成后引导卡片自动收起；卡片也可手动折叠，折叠状态会记忆在浏览器本地。

### 令牌授权

在「添加超级用户」步骤点击 **令牌授权**，提示框会显示当前有效的认证令牌，附带复制按钮与使用说明。将令牌发送到**任意平台的群聊**，机器人即自动完成初始化：

- 当前群加入**消息群**（`message_groups`）与**指令群**（`command_groups`）；
- 发送者加入**超级用户**（`SUPERUSERS`）。

令牌**即用即刷**：使用一次后立即刷新，旧令牌自动作废；机器人每次启动时也会在控制台打印当前令牌，无需登录 WebUI 也可使用。
...
- **开箱即用**：内置指令、默认渲染引擎与模板均已预置，无需额外配置。
