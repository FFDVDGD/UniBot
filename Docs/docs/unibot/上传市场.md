...
1. **打包**：把扩展源码压缩为 zip，确保**根目录包含 `Extension.toml`**。
2. **发布**：将 zip 作为资产上传到扩展仓库的 GitHub Release。
3. **登记**：在市场的 `extensions/<id>.json` 中登记元数据并提交 PR。
4. **合并**：工作流校验通过并合并后，自动生成 `extensions.json`，机器人即可发现你的扩展。
