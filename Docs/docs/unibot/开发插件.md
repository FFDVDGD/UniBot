...
3. 在扩展注册表（JSON 索引文件）中登记条目，包含元信息与 Release 资产地址。

安装流程（由 WebUI 触发，管理员确认）：

1. 从注册表选择版本，下载 Release 资产 zip。
2. 校验注册表提供的 SHA-256 校验和。
3. 校验 zip 根目录与清单中的 id 一致，拒绝绝对路径、`../` 路径与符号链接。
4. 解压到临时目录，完成全部校验后原子替换 `Extensions/<id>/`。
5. 重启后生效。

安装必须是可回滚事务：下载、校验、解压与清单验证全部在临时目录完成，任一步失败都不得改变当前版本。升级即下载新版本后原子替换目录；卸载即删除目录（均在 WebUI 操作，重启生效）。
