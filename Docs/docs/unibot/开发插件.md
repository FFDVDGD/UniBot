...
同一个扩展可以同时声明多种类型，例如 `WeatherExt` 就同时是 `api` 与 `command` 类型。
...
通过继承 `Service` 定义可复用的服务能力，用 `@extension.register_service` 登记。其它扩展或内置代码通过 `extension.api.get(name)` 获取。
...
```python
service = extension.api.get('greet')
...
`Extensions/` 目录下已内置 8 个单文件扩展（`About` / `Bound` / `Command` / `Help` / `List` / `Luck` / `Send` / `Server`）与目录型示例扩展 `WeatherExt`。它们是学习扩展开发的绝佳参考：

- `WeatherExt`：同时演示 API 服务注册与指令、业务配置（`config_model`）、`Extension.toml` 清单。
